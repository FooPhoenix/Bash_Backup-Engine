...
function checkFilesIntegrity
{
	...
	local canal_to_check canal_to_recheck canal_to_check_now
	local rsync_config=''
	...
	for try in {1..10}; do
		...
		while [[ 1 ]]; do
			...
			echo -en "$POS_CHECKED_1$(getCSI_CursorMove Up 4)sleep ${sleep_time}s       "

			while read -u ${canal} file_data file_flags file_size filename; do
			...
			done {canal}< <(rsync -vvitpoglDmc --files-from="$pathHWDR/ToCheckNow.files" --modify-window=5 \
	...
	echo -en "$POS_CHECKED_1$(getCSI_CursorMove Up 4)                           "

# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --

	(( count_files_checked_r > 0 )) && {
		while read -u ${canal} file_size filename; do
			((	count_size_checked_r -= file_size,  --count_files_checked_r,
				count_size_checked_p += file_size,  ++count_files_checked_p	))
...
# restore all previous section status		TODO do it if continue is choosen, and copy just Backup-Started here...
...
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
	...
	case $hostBackuped in
		'BravoTower')
			...
			declare hostSource="BravoTower"
			declare startingPoint="/"
			declare destinationPoint=''
			;;
		'Router')
			declare compressConfig=''
			declare hostSource="Router"
			declare startingPoint="/"
			declare destinationPoint=''
			;;
	esac # --bwlimit=30M
...
################################################################################################################################################################
	...
	echoStatPercent $stepA_R_CountFilesTotal $stepA_R_CountSizeTotal "$POS_TOTAL_1" "$S_NOWHI" $step1_CountFilesTotal $step1_CountSizeTotal
	echoStatPercent $stepA_P_CountFilesTotal $stepA_P_CountSizeTotal "$POS_TOTAL_2" "$S_NOWHI" $step1_CountFilesTotal $step1_CountSizeTotal
...
#==============================================================================================================================================================#
		...
		if checkSectionStatus 'Step_2-Current' $hostBackuped; then # TODO : do it at the same time that others period
...
#==============================================================================================================================================================#
				...
				done {canal}< <(cat "$pathHWDR/Excluded.files")
...
# 			declare -p PROGRESS_TOTAL_ITEM PROGRESS_TOTAL_SIZE PROGRESS_CURRENT_FILES_ITEM PROGRESS_CURRENT_FILES_SIZE >| "$pathHWDD/$VARIABLES_FOLDER/$hostBackuped-Step_3.var"
		...
		declare excludedFolder="$PATH_BACKUP_FOLDER/$TRASH_FOLDER/Rotation/$hostBackuped/Day-1"	# TODO BUG : Go to Current instead ?? A good way to clean it in the Rotation after...
...
#==============================================================================================================================================================#
			...
			if (( step1_CountFilesUpdated1 > 0 )); then
				...
				done {canal}< <(cat "$pathHWDR/Updated1.files")
...
#==============================================================================================================================================================#
			...
			if (( step1_CountFilesRemoved > 0 )); then
				...
				done  {canal}< <(cat "$pathHWDR/Removed.files")	# TODO : remove canal and put directly the file ?
...
function __change_log__
{
	...
	Next Objectives :	- Redesigne a bit the step 4.
						- Correct some bug with file type detection.
						- show memory usage by the working directory.
						- rotateFolder will rotate "Current" too.
						- Work a bit more on the statisics system (showing a better "total" while the backup).
						- Add a diffSize beside the fileSize.
